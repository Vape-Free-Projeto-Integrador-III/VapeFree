...
- **`ConnectionContext`** (`context/ConnectionContext.js`): `online`, `pendentes` (quantas escritas ainda não subiram), `falhas` (quantas a fila desistiu de enviar), `dadosIncompletos` (alguma leitura remota falhou sem espelho pra servir de reserva, então as telas estão mostrando o padrão vazio e não a conta), `sincronizarAgora`, `recarregarDados` (tenta a leitura de novo, via `precarregarEspelho`), `descartarFalhas` (limpa a lista de falhas depois que o usuário deu ciência). `falhas` e `dadosIncompletos` são atualizados por `assinarFalhas`/`assinarDadosIncompletos` — a leitura ou sincronização que falha costuma ser disparada pelo `storage.js`, fora daqui. Existe só pra UI — quem sincroniza de verdade é `utils/offline.js`, por baixo de `utils/storage.js`. Escuta o NetInfo (com um `estaOnline()` no mount, porque o NetInfo só avisa em mudança e o estado inicial é `online: true` — sem isso, abrir o app já sem rede escondia o banner), dispara a fila quando a rede volta e quando o app volta pro foreground, e aquece o espelho local depois do login (`precarregarEspelho`). Sem usuário logado, `pendentes` é sempre 0. Detalhe em [database.md](database.md).
