...
`CrisisHistory` não interfere nesse fluxo (só lista, edita e apaga sessão já salva): é aberta pelo atalho "Ver todas as crises" do `InsightsCard` (tela de Histórico) e sai por `goBack()`. Do estado vazio dela dá pra ir direto pro `Crisis`.
