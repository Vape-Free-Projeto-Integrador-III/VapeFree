...
Missões diárias e semanais que renovam sozinhas, dão XP e podem desbloquear conquistas. Lista de missões possíveis (código, não persistida): `utils/missions.js` → `MISSIONS`.

## Modelo — mesma ideia das conquistas

Missão **não** é contador. O progresso é sempre derivado de dados que já existem (`records`, `economy`, `crisisSessions`); o que fica salvo é só a lista de missões **concluídas**. Assim reabrir a tela, editar um registro ou trocar de aparelho não duplica nem apaga XP. Ver [database.md](database.md) para o shape gravado e `utils/xp.js` para como o XP entra na conta.

Cada entrada em `MISSIONS`:

```js
{ id, period: 'daily' | 'weekly', xp, icon, title, description,
  progress: (ctx) => ({ current, target }) }   // concluída quando current >= target
```

`ctx` vem de `buildMissionContext(records, economy, crisisSessions, today)` e traz `{ records, economy, crisisSessions, today, weekDays }`.

## Período e idempotência

- Diária: `periodKey` = `'YYYY-MM-DD'` do dia.
- Semanal: `periodKey` = data da segunda-feira daquela semana (`getWeekStart`).
...
1. A tela carrega `records`, `economy` e `crisisSessions`.
2. `checkAndCompleteMissions(records, economy, crisisSessions)` (`utils/storage.js`) avalia, **salva** as recém-concluídas e devolve só essas novas.
3. `checkAndUnlockAchievements(records, economy, completedMissions)` e `refreshXp(records, null, completedMissions)`.
4. `showRewards({ achievements, missions, gained })` (`context/XpToastContext.js`) mostra um toast por conquista/missão nova e um genérico com o XP restante.
5. Para exibir, a tela usa `checkMissions(ctx, completedMissions)` — devolve o estado de todas as missões do período com `current`/`target`/`completed`.
...
Adicione a entrada em `MISSIONS` com `progress` **pura** (sem I/O) e um `id` novo e estável — o `id` faz parte da chave do documento salvo, então renomear id perde histórico. Não mude o `xp` de uma missão esperando corrigir XP passado: `calcXp` usa o `xp` gravado na entrada concluída, de propósito.
