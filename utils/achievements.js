...
export const CONQUISTAS = [
    ...
    },
    {
        ...
        icone: '🛡️',
        condicao: (registros) => {
            const diasSemUso = new Set(registros.filter((r) => !r.used).map((r) => r.date)).size;
            return diasSemUso >= 7;
        },
    },
    {
...
export function listarDiasLimpos(registros) {
    const registrosPorData = agruparRegistrosPorData(Array.isArray(registros) ? registros : []);
    return Object.entries(registrosPorData)
        .filter(
            ([, registrosDoDia]) =>
                registrosDoDia.length > 0 &&
                !registrosDoDia.some((registro) => registro.used === true)
        )
        .map(([data]) => data)
        .sort();
}
