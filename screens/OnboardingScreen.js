...
export default function OnboardingScreen({ aoConcluir, navigation }) {
  ...
  function renderizarFormulario() {
    ...
    return (
      <View style={[styles.formulario, { width }]}>
        ...
        <Text style={[styles.label, { color: cores.text }]}>Quantas puxadas ele dá no total</Text>
        <TextInput
          style={estiloDoInput}
          placeholder="Ex: 600"
          placeholderTextColor={cores.textMuted}
        ...
        <Text style={[styles.label, { color: cores.text }]}>Quantos dias ele costuma durar</Text>
        <TextInput
          style={estiloDoInput}
          placeholder="Ex: 14"
          placeholderTextColor={cores.textMuted}
        ...
        {custo !== null && meta !== null ? (
          <View style={[styles.previa, { backgroundColor: cores.primaryLight }]}>
            <Text style={[styles.previaLinha, { color: cores.primaryDark }]}>
              R$ {custo.toFixed(4)} por puxada • meta de {Math.round(meta)} puxadas/dia
            </Text>
          </View>
        ) : null}
      </View>
  ...
  function renderizarFormularioDeMeta() {
    ...
    return (
      <View style={[styles.formulario, { width }]}>
        ...
        <Text style={[styles.texto, { color: cores.textSecondary }]}>
          Seu limite de puxadas desce um pouco a cada dia até o alvo — nada de cair tudo de uma vez.
        </Text>

        <Text style={[styles.label, { color: cores.text }]}>Quantas puxadas por dia hoje</Text>
        <TextInput
          style={estiloDoInput}
          placeholder="Ex: 100"
          placeholderTextColor={cores.textMuted}
        ...
        <Text style={[styles.label, { color: cores.text }]}>Quer chegar a quantas por dia</Text>
        <TextInput
          style={estiloDoInput}
          placeholder="Ex: 10"
          placeholderTextColor={cores.textMuted}
  ...
  if (extras === null) {
    return (
      <View style={[styles.container, styles.carregando, { backgroundColor: cores.background }]}>
  ...
  return (
    <View style={[styles.container, { backgroundColor: cores.background }]}>
      ...
      <TouchableOpacity
        style={[styles.botao, { backgroundColor: cores.primary }, salvando && styles.botaoDesativado, SOMBRA.media]}
        onPress={avancar}
        disabled={salvando}
        activeOpacity={0.85}
      >
      ...
      {noFormulario ? (
        <TouchableOpacity onPress={seguir} disabled={salvando} style={styles.depois}>
          <Text style={[styles.depoisTexto, { color: cores.textMuted }]}>Fazer isso depois</Text>
        </TouchableOpacity>
      ) : null}
    </View>
  );
